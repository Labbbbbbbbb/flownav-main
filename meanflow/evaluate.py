...
def evaluate(
    ...
    with tqdm.tqdm(
        ...
        for i, data in enumerate(tepoch):
            ...
            # MeanFlow one-step eval: x0 = n - u(n, t=1, r=0)
            with torch.no_grad():
                n = torch.randn(naction.shape, device=device)
                t_ones = torch.ones(B, device=device)

                # 1. RANDOM MASK
                u = ema_model("noise_pred_net", sample=n, timestep=t_ones, global_cond=rand_mask_cond)
                rand_mask_loss = nn.functional.mse_loss(n - u, naction)

                # 2. No mask
                u = ema_model("noise_pred_net", sample=n, timestep=t_ones, global_cond=obsgoal_cond)
                no_mask_loss = nn.functional.mse_loss(n - u, naction)

                # 3. Goal mask
                u = ema_model("noise_pred_net", sample=n, timestep=t_ones, global_cond=goal_mask_cond)
                goal_mask_loss = nn.functional.mse_loss(n - u, naction)
